...
$currentNav='cotacoes_convidadas';
?>
<!DOCTYPE html>
...
<title>Cotações Convidadas - <?php echo htmlspecialchars($branding['app_name'] ?? 'Dekanto'); ?></title>
...
<div class="container py-4" id="lista-convites-container">
  <div class="card-matrix mb-4">
    <div class="card-header-matrix d-flex justify-content-between align-items-center">
      <span><i class="bi bi-envelope-open me-2"></i>Cotações em que fui Convidado</span>
      <div class="filtros-inline d-flex align-items-center gap-2">
        <input type="text" id="filtroBusca" class="form-control form-control-sm" placeholder="Buscar por convite ou requisição...">
        <select id="filtroStatusConvite" class="form-select form-select-sm">
          <option value="">Status convite (todos)</option>
        </select>
    ...
    <div class="table-responsive">
      <table class="table table-hover align-middle mb-0">
        ...
        <tbody id="tbody-convites"><tr><td colspan="9" class="text-center text-secondary py-4">Carregando...</td></tr></tbody>
      </table>
    </div>
    <div class="d-flex flex-column flex-md-row gap-2 justify-content-between align-items-md-center px-3 py-3 border-top border-dark-subtle">
      ...
      <nav aria-label="Paginação de convites" data-bs-theme="light">
...
<script>
const API_URL='../../api/fornecedor/cotacoes_convidadas.php';
...
function fmtDate(d){if(!d)return '–';const normalized=d.includes('T')?d:d.replace(' ','T');const dt=new Date(normalized);if(Number.isNaN(dt.getTime()))return d;return dt.toLocaleDateString('pt-BR',{day:'2-digit',month:'2-digit'})+' '+dt.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'});}
function badgeColor(st){switch((st||'').toLowerCase()){case 'enviado':return 'warning';case 'respondido':return 'success';case 'cancelado':return 'secondary';case 'expirado':return 'danger';default:return 'secondary';}}
const state={
  rows:[],
...
const filtros={
  ...
  convite:document.getElementById('filtroStatusConvite'),
  cotacao:document.getElementById('filtroStatusCotacao')
};
const ui={
  tbody:document.getElementById('tbody-convites'),
...
function buildParams(page){const params=new URLSearchParams();params.set('page',Math.max(1,page));params.set('per_page',state.meta.per_page||DEFAULT_PER_PAGE);const termo=(filtros.busca?.value||'').trim();if(termo){params.set('busca',termo);}const conviteVal=filtros.convite?.value||'';if(conviteVal){params.set('status_convite',conviteVal);}const cotacaoVal=filtros.cotacao?.value||'';if(cotacaoVal){params.set('status_cotacao',cotacaoVal);}return params;}
function renderRows(){if(!ui.tbody)return;const rows=Array.isArray(state.rows)?state.rows:[];if(!rows.length){ui.tbody.innerHTML='<tr><td colspan="9" class="text-center text-secondary py-4">Nenhum convite encontrado.</td></tr>';return;}ui.tbody.innerHTML=rows.map(r=>{const stConv=`<span data-bs-theme="light" class="badge bg-${badgeColor(r.convite_status)}-subtle text-${badgeColor(r.convite_status)}-emphasis">${r.convite_status||'-'}</span>`;const stCot=r.cotacao_status?`<span data-bs-theme="light" class="badge bg-info-subtle text-info-emphasis">${r.cotacao_status}</span>`:'-';const prop=r.minha_proposta_id?`#${r.minha_proposta_id}`:'-';const expira=fmtDate(r.expira_em);const link=r.cotacao_id?`<a class="btn btn-sm btn-matrix-primary" href="cotacao.php?id=${r.cotacao_id}"><i class="bi bi-arrow-right"></i></a>`:'<span class="text-secondary" style="font-size:.75rem">Sem cotação</span>';return `<tr>
        <td>#${r.convite_id}</td>
        <td>#${r.requisicao_id}</td>
        <td>${r.cotacao_id?('#'+r.cotacao_id):'-'}</td>
        <td>${r.cotacao_rodada||'-'}</td>
        <td>${stConv}</td>
        <td>${stCot}</td>
        <td>${prop}</td>
        <td>${expira}</td>
...
function preencherFiltro(selectEl,placeholder,lista){if(!selectEl)return;const selected=(selectEl.value||'').toLowerCase();const options=[`<option value="">${placeholder}</option>`];(Array.isArray(lista)?lista:[]).forEach(item=>{if(!item||!item.value)return;const value=String(item.value).toLowerCase();options.push(`<option value="${value}">${item.label||item.value}</option>`);});selectEl.innerHTML=options.join('');if(selected){const exists=(lista||[]).some(item=>String(item.value).toLowerCase()===selected);if(exists){selectEl.value=selected;}}
}
async function carregar(pageOverride){const page=Math.max(1,pageOverride||state.meta.page||1);setTableLoading();try{const params=buildParams(page);const resp=await fetch(`${API_URL}?${params.toString()}`,{cache:'no-store'});if(!resp.ok)throw new Error('Erro ao carregar convites');const payload=await resp.json();state.rows=Array.isArray(payload?.data)?payload.data:[];const meta=payload?.meta||{};state.meta={
    ...
    state.statusConviteOptions=Array.isArray(meta.status_convite_options)?meta.status_convite_options:[];
    state.statusCotacaoOptions=Array.isArray(meta.status_cotacao_options)?meta.status_cotacao_options:[];
    renderRows();
    preencherFiltro(filtros.convite,'Status convite (todos)',state.statusConviteOptions);
    preencherFiltro(filtros.cotacao,'Status cotação (todos)',state.statusCotacaoOptions);
    renderPagination();
  }catch(err){if(ui.tbody){ui.tbody.innerHTML='<tr><td colspan="9" class="text-center text-danger py-4">Erro ao carregar convites.</td></tr>';}
    if(ui.pagination){ui.pagination.innerHTML='';}
    if(ui.hint){ui.hint.textContent='';}
    showToast('Não foi possível carregar os convites agora.','danger');
    console.error(err);
  }
}
...
document.addEventListener('DOMContentLoaded',()=>{
  ...
  filtros.convite?.addEventListener('change',handleFiltroChange);
